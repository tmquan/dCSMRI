...
def get_cae():
	...
	num_filter = 10*20
	encoder = tflearn.conv_2d(encoder, num_filter*1, 3, activation='relu', regularizer='L2')
	...
	encoder = tflearn.conv_2d(encoder, num_filter*2, 3, activation='relu', regularizer='L2')
	scale_1 = encoder
	# encoder = tflearn.layers.normalization.batch_normalization(encoder)
	encoder = tflearn.max_pool_2d(encoder, 2)
	# encoder = tflearn.dropout(encoder, 0.75)
	...
	encoder = tflearn.conv_2d(encoder, num_filter*4, 3, activation='relu', regularizer='L2')
	scale_2 = encoder
	# encoder = tflearn.layers.normalization.batch_normalization(encoder)
	encoder = tflearn.max_pool_2d(encoder, 2)
	# encoder = tflearn.dropout(encoder, 0.75)
	...
	encoder = tflearn.conv_2d(encoder, num_filter*8, 3, activation='relu', regularizer='L2')
	scale_3 = encoder
	# encoder = tflearn.layers.normalization.batch_normalization(encoder)
	encoder = tflearn.max_pool_2d(encoder, 2)
	# encoder = tflearn.dropout(encoder, 0.75)
	...
	# encoder = tflearn.conv_2d(encoder, num_filter*12, 3, activation='relu', regularizer='L2')
	...
	# decoder = tflearn.conv_2d(decoder, num_filter*8, 3, activation='relu', regularizer='L2')
	...
	# decoder = tflearn.conv_2d(decoder, num_filter*4, 3, activation='relu', regularizer='L2')
	...
	# decoder = tflearn.conv_2d(decoder, num_filter*2, 3, activation='relu', regularizer='L2')
	...
	# decoder = tflearn.conv_2d(decoder, num_filter*1, 3, activation='relu', regularizer='L2')
	...
	# decoder = tflearn.conv_2d(decoder, 20, 1, activation='relu', regularizer='L2')
	decoder = tflearn.conv_2d_transpose(decoder, 
									 nb_filter=20, 
									 filter_size=3, 
									 activation='relu',
									 regularizer='L2',
									 output_shape=[256, 256])
	# tflearn.add_weight_regularizer(decoder, 'L1', weight_decay=0.001)
	tflearn.losses.L1(decoder)
	# decoder = tflearn.layers.conv.upscore_layer(decoder, 
	# 						 num_classes=256, 
	# 						 kernel_size=3, 
	# 						 shape=[1, 256, 256, 20]
